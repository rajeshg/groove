...
test.describe("Board Workflows", () => {
      ...
      if (await cardInput.isVisible()) {
        ...
        if (cardVisible) {
          ...
          const cardLocator = page
            .locator("li[data-card-id]")
            .filter({ hasText: /Design.*landing/i })
            .first();
    ...
    if (await addCardButton.isVisible()) {
      ...
      if (await cardInput.isVisible()) {
        ...
        await commentInput.fill(
          "This bug affects login with special characters"
        );
        await commentInput.press("Enter");

        // Wait for comment to appear
        await page.waitForTimeout(500);
        await expect(page.locator("body")).toContainText("special characters");

        // Add second comment
        await commentInput.fill("Reproduced on Chrome and Firefox");
        await commentInput.press("Enter");
        ...
        const secondCommentContainer = page
          .locator("text=Chrome and Firefox")
          .locator("..")
          .locator("..");
        const deleteButton = secondCommentContainer.getByRole("button", {
          name: /Delete/i,
        });

        const deletePromise = page.waitForResponse(
          (resp) =>
            resp.url().includes("/resources/delete-comment") &&
            resp.status() === 200,
          { timeout: 5000 }
        );
  ...
  test("should create a new column", async ({ page }) => {
    ...
    if (await newColumnSection.isVisible()) {
      const addCardButton = newColumnSection.getByRole("button", {
        name: /add.*card/i,
      });
      if (await addCardButton.isVisible()) {
        await addCardButton.click();
