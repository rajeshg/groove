...
test.describe("Card Workflows - Comprehensive", () => {
  ...
  test("should move card between columns", async ({ page }) => {
    ...
    if (buttonCount > 1) {
      ...
      const inProgressButton = page
        .locator("button")
        .filter({ hasText: /In Progress|Done/i })
        .first();
  ...
  test("should cancel card creation", async ({ page }) => {
    ...
    const cardLocator = page
      .locator("[data-card-id]")
      .filter({ hasText: "This card will be cancelled" });
  ...
  test("should display card metadata (creation date, creator)", async ({
    ...
    const cardLocator = page
      .locator("[data-card-id]")
      .filter({ hasText: "Card with metadata" });
    await expect(cardLocator).toBeVisible();
    ...
    const hasDateInfo =
      (await page
        .locator("text=/created|updated|today|yesterday|ago/i")
        .count()) > 0;
  ...
  test("should handle empty card title validation", async ({ page }) => {
    ...
    const validCard = page
      .locator("[data-card-id]")
      .filter({ hasText: "Valid Card Title" });
  ...
  test("should persist card edits across page refresh", async ({ page }) => {
    ...
    const cardLocator = page
      .locator("[data-card-id]")
      .filter({ hasText: "Persistent card title" });
    await expect(cardLocator).toBeVisible();
    ...
    const updatePromise = page.waitForResponse(
      (resp) =>
        resp.url().includes("/resources/update-card") && resp.status() === 200,
      { timeout: 5000 }
    );
    ...
    const editedCardLocator = page
      .locator("[data-card-id]")
      .filter({ hasText: "Edited persistent title" });
