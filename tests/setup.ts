...
const now = new Date()
const timestamp = now.toISOString().replace(/[:-]/g, '').replace(/\..+/, '') + '-' + now.getMilliseconds().toString().padStart(3, '0')
...
if (fs.existsSync(mainDbPath)) {
  try {
    ...
    console.log("⚠️  Could not copy main database, test will create fresh database")
  }
}
