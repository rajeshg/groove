...
function Avatar({ label, bgColor }: { label: string; bgColor?: string }) {
  ...
  return (
    <div
      className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold text-white flex-shrink-0 shadow-sm transition-transform hover:scale-110"
...
export function CardMeta({
  ...
  const createdByName = useMemo(() => {
    if (createdByUser) {
      // Create a safe object for getDisplayName that always has email
      const userForDisplay = {
        firstName: createdByUser.firstName,
        lastName: createdByUser.lastName,
        email: createdByUser.email || "",
      };
      return getDisplayName(userForDisplay);
    }
  ...
  return (
    <div className="mt-3 pt-3">
      <div
        className="grid text-xs uppercase font-bold tracking-widest"
        style={{
      ...
      >
        ...
        >
          <span className="truncate">
            ADDED <span className="font-bold ml-1">{createdTimeText}</span>
          </span>
        </span>
        ...
        >
          {hasUpdate ? (
            <span className="truncate">
              ...
              <span className="font-bold">{lastActiveTimeText}</span>
            </span>
          ) : (
        ...
        <span
          className="whitespace-nowrap border-r border-slate-300 dark:border-slate-700 pr-3 pt-[0.75ch] capitalize text-slate-700 dark:text-slate-300 font-semibold flex items-center min-w-0"
        ...
        <span
          className="whitespace-nowrap pl-3 pt-[0.75ch] text-right capitalize text-slate-700 dark:text-slate-300 font-semibold flex items-center justify-end min-w-0"
