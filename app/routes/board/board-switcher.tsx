...
export function BoardSwitcher({ currentBoardId, currentBoardName, allBoards }: BoardSwitcherProps) {
   const [isOpen, setIsOpen] = useState(false);
   const dropdownRef = useRef<HTMLDivElement>(null);

   // Close dropdown when clicking outside
   const handleClickOutside = (e: React.MouseEvent) => {
     if (dropdownRef.current && !dropdownRef.current.contains(e.currentTarget as Node)) {
       setIsOpen(false);
     }
   };
   ...
   return (
     <div className="relative" ref={dropdownRef} onClick={() => setIsOpen(!isOpen)}>
       ...
       >
         ...
         <span className="text-lg font-bold truncate max-w-xs">{currentBoardName}</span>
