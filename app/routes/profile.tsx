...
export default function Profile({ loaderData }: Route.ComponentProps) {
  const { email, firstName, lastName } = loaderData;
      ...
      <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700 p-10 flex flex-col items-center relative overflow-hidden">
            ...
            {/* Avatar */}
            <div
              className="w-32 h-32 rounded-full flex items-center justify-center text-white text-4xl font-bold mb-6 shadow-inner"
              style={{ backgroundColor: avatarColor }}
            >
              {initials}
            </div>

            <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-2 text-center">
              {displayName}
            </h1>
            <a
              href={`mailto:${email}`}
              className="text-blue-500 hover:underline mb-10 font-medium"
            >
              {email}
            </a>
