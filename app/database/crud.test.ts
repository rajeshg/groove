...
afterAll(async () => {
  ...
  if (testComment)
    await prisma.comment
      .delete({ where: { id: testComment.id } })
      .catch(() => {});
  if (testItem)
  ...
  if (testColumn)
    await prisma.column
      ...
      .catch(() => {});
  if (testBoard)
    await prisma.board.delete({ where: { id: testBoard.id } }).catch(() => {});
  if (testAccount)
...
describe("Item (Card) CRUD", { timeout: 30000 }, () => {
  it("CREATE: should create an item with beautiful ID", async () => {
    if (!testBoard || !testColumn)
      throw new Error("Test board/column not created");
  ...
  it("CREATE: should create item with metadata tracking", async () => {
    if (!testBoard || !testColumn || !testAccount)
      throw new Error("Prerequisites not created");
  ...
  it("UPSERT: should handle upsertItem with existing ID (update case)", async () => {
    if (!testItem || !testBoard || !testColumn || !testAccount)
      throw new Error("Prerequisites not created");
...
describe("CRUD Integration", { timeout: 30000 }, () => {
  it("should create full workflow: Board -> Column -> Item -> Comment", async () => {
    ...
    expect(fullBoard?.columns[0]?.items[0]?.comments[0]?.content).toBe(
      "Integration comment"
    );
